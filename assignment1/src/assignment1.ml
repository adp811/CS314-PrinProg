...
(* Helper Functions *)
let rec sum_helper l = 
  match l with
  | [] -> 0
  | (h :: t) -> h + (sum_helper t);;

let rec ap_helper num count = 
  if num < 10 then
    count
  else 
    ap_helper (sum_helper (digitsOfInt (num))) (count + 1);;
    
let rec dr_helper num =
  if num < 10 then
    num
  else
    dr_helper (sum_helper (digitsOfInt (num)));;
   
let additivePersistence n = ap_helper n 0;;
  
let digitalRoot n = dr_helper n;;
...
let main () =
  ...
  let _ =
    try
      ...
      assert (remove_stutter [1;2] = [1;2]);
      assert (remove_stutter [1;2;3] = [1;2;3])
    with e -> (error_count := !error_count + 1; print_string ((Printexc.to_string e)^"\n")) in
